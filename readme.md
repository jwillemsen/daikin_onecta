[![Open your Home Assistant instance and open a repository inside the Home Assistant Community Store.](https://my.home-assistant.io/badges/hacs_repository.svg)](https://my.home-assistant.io/redirect/hacs_repository/?owner=jwillemsen&repository=daikin_onecta)
...
[![BuyMeCoffee](https://img.shields.io/badge/buy%20me%20a%20coffee-donate-yellow.svg?style=for-the-badge)](https://www.buymeacoffee.com/jwillemsen)
...
Only when the redirect back to your Home Assistant doesn't work first try to install/enable [My Home Assistant](https://www.home-assistant.io/integrations/my/). When that also doesn't work you can try `<HOME_ASSISTANT_URL>/auth/external/callback` as the redirect URI instead. The `<HOME_ASSISTANT_URL>` must exactly match the base URL you use to access Home Assistant, including the scheme (`http` or `https`) and only including a port when your Home Assistant instance is actually served on that port. For example, use `http://homeassistant.local/auth/external/callback` when you access Home Assistant as `http://homeassistant.local`, or `http://192.168.0.2:8123/auth/external/callback` when you access it as `http://192.168.0.2:8123`.
...
Current poll limits can be found under [system health](https://my.home-assistant.io/redirect/system_health).
